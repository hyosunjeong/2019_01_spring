...
public class FileUpService {
	...
	// 파일을 업로드하기 위해 서버의 실제 디렉토리 경로를 
	// 추출하는데 사용할 servletContext를 선언
	...
	public String upload(MultipartFile file) {
		...
		// 실제 업로드한 파일에 여러가지 문제가 발생하면
		// 더이상 진행하지 말라
		if(file.isEmpty() || file == null) return null;
		
		
		// 원래의 파일이름으로 파일을 업로드할 경우 
		// 같은 이름이 있을 경우 앞에 업로드된 파일이 나중 파일로 교체되버리는 현상이 있어서
		// 실제로 파일을 업로드 할 때는 임의의 이름으로 변경해주는 것이 좋다.
