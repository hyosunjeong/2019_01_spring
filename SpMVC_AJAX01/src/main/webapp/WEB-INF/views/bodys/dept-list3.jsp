...
/* tr의 각 요소들에 대한 셋팅*/
/* 홀수 tr과 짝수 tr의 색깔을 달리해서 라인 구분 설정*/
...
$(function(){
	...
	/* js 배열은 동적 배열
		초기에 배열을 생성한 후 자유롭게 데이터들을 추가, 삭제 가능
		1. 배열 생성 : var arr = Array()
		2. 배열 추가 : arr.push("값")
	*/
	...
	$(".d-tr").click(function(){
		...
		// dcode, dname, dceo를 JSON 형식으로 만들어서
		// 배열에 추가 
		let json_data = {
					d_code:dcode,
					d_name:dname,
	...
	$("#btn-send").click(function(){
		...
		*/
		...
		$.ajax({
			...
			data :JSON.stringify(deptList), // 데이터를 JSON 형태로 서버에 보내라
...
<table>
	...
	<c:choose>
		...
		<tr class="d-tr" data-d-code="${dVO.d_code}"
			data-d-name="${dVO.d_name}" data-d-ceo="${dVO.d_ceo}">
			<td>${i.count}</td>
			<td>${dVO.d_code}</td>
			<td>${dVO.d_name}</td>
			<td>${dVO.d_ceo}</td>
