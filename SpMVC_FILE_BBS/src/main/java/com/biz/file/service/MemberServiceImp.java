...
/*
 * 구현체 
 * 	Service interface를 implement 하여 구현한 실체(메서드의 구체적인 기능들을 코딩한 클래스)
 * 
 * 인터페이스와 구현체의 사용
 * 일반적으로 클래스를 사용하기 위해서는 아래와 같이 생성
 * 		클래스 객체 = new 클래스()
 * 		이름				생성자
 * 
 * 인터페이스와 구현체일 경우는
 ...
 * 		리스트형 인터페이스 			구현체생성자
 * 		※ List memberList = new List 형식은 사용불가! 
 * 
 * 자바에서는 어떤 클래스의 인터페이스가 있는 경우는 가능하면
 * 인터페이스로 선언을 하고 클래스로 생성을 하라고 한다.
 * 
 * 일반적으로 아래와 같고 
...
public class MemberServiceImp implements MemberService {
	...
	public int save(@Valid MemberVO memberVO) {
		...
		if(vo==null) { // 없는 id이면
			...
			String encodoString = vo.getM_password();
			
			// 비밀번호가 일치하지 않으면 update가 되지 않도록 한다.
			if(encoder.matches(rawString, encodoString)) {
