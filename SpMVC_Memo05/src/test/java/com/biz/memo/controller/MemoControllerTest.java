...
public class MemoControllerTest {
	...
	public void testAdd() throws Exception{
		...
		// Controller를 test 하기 위한 코드
		mockMvc.perform(get("/memo/add"))
				.andExpect(status().isOk())
				.andDo(print());
		
		// Controller의 memo/add가 호출되었을 때
		// Serivce의 add method가 잘 실행되었는지 보기
		
		verify(mService,times(3)).add();
	}
	...
	public void testGetName() throws Exception{
		...
		// 값을 저장하는 (setter) 메서드가 1번씩만 실행 되었는가?
		verify(vo,times(1)).setSubject("메모쓰기");
		verify(vo,times(1)).setMemo("오늘의 메모");

		// 메모쓰기가 1번 이상 setting 되었는가
		// verify(vo,atLeastOnce()).setSubject("메모쓰기");
		
		// 임의의 문자열이 1번이상  setting 되었는가
		// verify(vo,atLeastOnce()).setSubject(any(String.class));
