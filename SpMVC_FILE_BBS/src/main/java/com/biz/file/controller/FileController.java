...
public class FileController {
	...
	/*
	 * @Controller에서 
	 * @ResponseBody가 설정된 method가 String(문자열)을 return하면
	 * 문자열 자체가 client에게 전송되어 WebBroser에게 문자열이 찍히게 된다.
	 * 
	 * @ResponseBody가 설정되지 않은 method가 String(문자열)을 return하면
	 * WEB-INF/views/문자열.jsp 파일을 찾아 읽어서 렌더링 한 후 client에게 전송한다.
	 * 
	 * 또한
	 * @RestController로 설정하게 되면
	 * 모든 method가 자동으로 @ResponseBody로 설정 된다.
	 * 
	 * 그리고,
	 * jackson-bind를 Dependencies에 추가해 주게 되면
	 * @ResponseBody는 String 뿐만 아니라
	 * List나 사용자가 직접 생성한 클래서 type을 return할 수 있고
	 * 해당 내용은 적절하게 변환되어 json 형태로 client에게 전송된다.
	 * 
	 */
