...
public class MyController {
	 ...
	 * ms 객체를 생성(초기화)하여 사용할 준비를 하라 라는 지시어
	 */
	@Autowired
	MyService ms;
	...
	public String calc(int intNum1, int intNum2, Model model) {
		...
		int intDevide = ms.divide(intNum1, intNum2);
	...
	public String calc(@ModelAttribute CalcVO vo, Model model) {
		...
		int intDevide = ms.divide(vo.getIntNum1(), vo.getIntNum2());
	...
	public String inOut(@ModelAttribute InoutVO vo, Model model) {
		
	
		model.addAttribute("INOUT", vo);
