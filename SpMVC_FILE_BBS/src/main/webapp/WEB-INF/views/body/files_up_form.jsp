...
$(function(){
	$(".drag_area").on("dragover",function(e){
		$('h3').text('파일을 내려 놓으세요')
		return false // 코드 진행을 중단하라
	})
	
	// drop event가 발생을 하면
	// 파일을 끌어서 drag_area box에 놓으면 
	// drop event가 발생하고
	// 끌어놓은 파일에 대한 정보가 e 매개변수에 담기게 된다
	$('.drag_area').on('drop',function(e){
		$('h3').text('파일을 등록하는중')
		
		let files = e.originalEvent.dataTransfer.files
		
		// ajax로 전송하기 위해 formData객체를 생성한다
		let fData = new FormData();
		
		// fData에 파일정보를 추가
					// file : controller에서 사용할 변수의 이름
		...
		$.ajax({
			...
			success:function(result){
				// JSON형태로 수신한 문자열을 이요해서
				// HTML 코드를 생성한 후 본문에 그려보자
				if(result == null){
					$("h3").text("파일 업로드 오류")
				}else{
					for(let i = 0; i<result.lenght; i++){
						// 특정한 영역에 문자열이나 html코드를 추가하는 method
						$('#images').append(
							$('<img/>',{
