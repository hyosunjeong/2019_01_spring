...
public class LoginController {
				...
				HttpSession session	)
	{
		...
		// mList에는 userid에 해당하는 사용자(들)이 포함되어 있다
		boolean login_ok = false;
		if(mList != null) {
			for(MemberVO vo : mList) {

				// vo.m_password에 담겨있는 비밀번호는 Bcrypt 암호문이다
				// 이 암호문을 로그인 폼에서 입력한 비밀번호와 == 이나 equels() 등으로는 비교할 수 없다
				// 그래서 Bcrypt... 클래스에 있는 암호 비교 method를 통해서 값을 비교해야 한다
				...
				if(passwordEncoder.matches(password, vo.getM_password())) {
					...
					/*
					 * login 폼에서 req된 memberVO에는 m_userid와 m_password만 담긴 상태이다
					 * 그래서 DB로부터 조회된 vo데이터를 다시 memberVO에 세팅하여
					 * 이름, 전화번호, 기타 정보를 조회할 수 있도록 한다
					 *  
					 */
					memberVO = vo;
		...
		String ret = "redirect:/";
		// 정상적으로 login 정보가 확인되면 login_ok값이 true일 것이다
		if(login_ok) {
			// 이 프로젝트가 서버에서 실행되고 있는 동안에
			// 같은 웹브라우저 페이지에서 req, res 중일 때 어디에서나
			// LOGIN이라는 변수를 통해서 memberVO 내용을 조회할 수 있게 된다
	...
	public String logout(HttpSession session) {
		...
		// 혹시 모를 시스템에서 사용하는 세션 정보가 있을 경우
		// 로그인정보만 삭제한다
		session.setAttribute("LOGIN_INFO", null); // 값을 null로 해서 삭제 된 것처럼
		session.removeAttribute("LOGIN_INFO"); // 값을 완전히 삭제
