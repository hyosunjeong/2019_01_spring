...
public interface MemoDao {
	...
	/*
	 * tbl_memo 테이블로부터 데이터 리스트를 가져올 때
	 * memoVO의 files 맴버변수에 tbl_files 테이블로부터
	 * tbl_memo와 Relation(Join)된 데이터를 
	 * 같이 가져오기 위하여 설정한 Annotation
	 * 
	 */
	@Results({
	...
	/*
	 * 1. tbl_memo에서 data를 select하라
	 * 2. tbl_memo data중에서 id칼럼의 값을 내가 사용하겠다
	 * 3. getFiles 메서드를 호출하는데 
	 * 4. 매개변수(parent_id)로 전달을 하겠다
	 * 5. getFiles는 parent_id를 기준으로 데이터를 select한다음에
	 * 6. List<FileVO>로 return할 것이다
	 * 7. 그 결과를 memoVO의 files 변수에 받아라
	 */
	@Select(" SELECT * FROM tbl_memo WHERE m_auth = #{m_userid} ") //1	
	@Results({
			@Result(property="id",column="id"), //2
			@Result(column="parent_id", property="files", //4,7
				javaType=List.class,
				many=@Many(select="getFiles") //3
			)
	})
	...
	/*
	 * InsertProvider 문장이 실행되기 전에 (before가 true 이므로)
	 * SELECT SEQ_MEMO.NEXTVAL FROM DUAL을 실행해서 그 결과 값을
	 * id라는 변수에 담아라
	 * 이 때 이 id 변수의 자료형은 Long 형이다.
	 */
