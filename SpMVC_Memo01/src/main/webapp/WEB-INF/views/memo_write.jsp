...
function myfunc(){
	// alert("반갑습니다")
	/*
		document(HTML문서)에서 id가 m_message로 설정된 tag를 찾아서
		그곳의 문자열을 반갑습니다 로 설정하라
	*/
	document.getElementById("m_message").innerHTML="반갑습니다"
	
	/*
		var : JavaScript(JS)에서 변수를 선언하는 keyword
		String, int 이런 형(type)들이 없이
		var, let라는 keyword로 시작되면 변수 선언문이 된다.
	*/
	...
	if(s_subject==""){
		...
		document.getElementById("m_subjcet").focus()
		return false
	}
	/*
		form에 있는 메모저장 button의 click을 무력화 시키고
		JS 함수를 실행 하도록 설정을 변경 했기 때문에
		form에 입력된 데이터들을 서버로 전송하기 위해
		마지막에 submit을 강제로 실행시켜줘야한다.
	*/
...
<section>
	<form action ="memo_write" id="memo_form" method="POST">
		...
		<input type="text" id="m_auth" name="m_auth" placeholder="작성자 이름을 입력하세요"
				class="w3-input w3-border w3-hover-light-gray">
		
		<label for="m_date">작성일자</label>
		<input type="text" id="m_date" name="m_date" placeholder="작성일자를 입력하세요" 
				class="w3-input w3-border w3-hover-light-gray">
		
		<label for="m_subject">제목</label>
		<input type="text" id="m_subject" name="m_subject" placeholder="제목을 입력하세요"
				class="w3-input w3-border w3-hover-light-gray">
		
		<label>메모</label>
		<textarea rows="5" id="m_text" name="m_text" placeholder="메모를 입력하세요" 
