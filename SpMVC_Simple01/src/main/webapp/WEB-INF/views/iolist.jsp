...
<body>
	<header class="w3-green">
		<h3 class="w3-center w3-whilte">매입매출리스트</h3>
	</header>
	<section>
						...
						<tr>
							...
							<c:if test="${VO.io_check=='1'}">
								<td class="w3-text-right">
									<fmt:formatNumber value="${VO.io_price}" type="number" pattern=".0" />
								</td>
								<td class="w3-text-right">
									<fmt:formatNumber value="${VO.io_price * VO.io_su}" type="number" /></td>
								<td></td><td></td>
							</c:if>
							...
							<c:if test="${VO.io_check=='2'}">
								...
								<td class="w3-text-right">
								<fmt:formatNumber value="${VO.io_price * VO.io_su}"	type="currency" /></td>
							</c:if>
						</tr>
					</c:forEach>
				</c:otherwise>
...
<!--  
	...
	fmt FormatNumber var = "값.값" type="percent" 만약 0.3이면 30%로 표현
												  만약 1.2이면 120%로 표현
	fmt FormatNumber var = "값.123412" pattern=".00" 숫자형으로 표현하되 소수점이하 2자리까지 표시
	fmt FormatDate pattern ="yyyy/mm/dd" : 값이 날짜 형일 경우
