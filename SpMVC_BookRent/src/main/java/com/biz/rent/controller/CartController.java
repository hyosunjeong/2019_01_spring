	...
	public String cart(@ModelAttribute("rentVO") RentVO rentVO, Model model, HttpSession httpSession) {
		...
		try {
			...
			if(returnCart != null) {
				...
				// returnCart의 0번째 요소에 cart_seq는
				// 1이 되고 순서대로 2,3,4,5...값이 연속해서 부여된다.
				// cart_seq에 저장된 값은 
				// 리스트의 순서가 바뀌어도 변하지 않는 고유 값이 될것이다.
