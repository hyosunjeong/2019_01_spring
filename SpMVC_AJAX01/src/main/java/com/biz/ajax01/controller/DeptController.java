...
DeptService dService; 
	...
	@ResponseBody
	@RequestMapping(value="dept",method=RequestMethod.POST,produces="text/plan;charest=utf8")
	 ...
	 * 문자열로 return 하려면
	 * @ResponseBody를 설정해주면 된다.
	 * 
	 * 그런데 Ajax로 호출된 결과를 다른용도로 사용하려면
	 * JSON 형태로 return을 해주는 것이 좋다.
	 * 
	 * 하지만
	 * JAVA(spring)에서는 기본적으로 JSON 데이터 형태를 지원하지 않는다.
	 * 
	 * 다행히 spring에는 JSON을 쉽게 사용할 수 있는 
	 * 통로가 마련되어 있고, 한가지의 3rd part lib만 설정 해주면 된다.
	 * 
	 */

		
	@ResponseBody
	@RequestMapping(value="dept.JSON",method=RequestMethod.POST,produces="application/json;charest=utf8")
