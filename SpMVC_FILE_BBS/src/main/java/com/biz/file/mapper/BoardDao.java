...
public interface BoardDao {
	 ...
	 * 그래서 날짜 시간을 역순(내림차순)으로 정렬해야한다.
	 * ORDER BY는 기본값이 오름차순 정렬이다. 생략하면 ASC가 생략된것과 같다.
	 * 최신글을 제일 먼저 보이려면 ORDER BY DESC로 설정한다.
	 * 
	 */
	...
	/*
	 * @SelectKey는 @InsertProvider가 실행되기 전(before=true)이나
	 * 		실행된 후(before=false)에 statement에 지정된 SQL을 실행하라
	 * 
	 * 이 경우는 before=true이므로 Insert 이전에 실행을 해서
	 * keyProperty로 지정된 변수에 저장
	 * 이 때 id의 자료형은 Long형이다.
	 */
